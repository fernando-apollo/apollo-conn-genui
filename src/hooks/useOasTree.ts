...
export function useOasTree(parser: OasGen, onChange: (paths: string[], schema: string) => void) {
    ...
    const createNode = (item: IType, root: Node | undefined): Node => {
        const isScalarOrEnum = item.id.startsWith('prop:scalar') || item.id.startsWith('enum:');
    ...
    const onCheck = (values: Key[] | CheckedProps): void => {
        ...
        if (typeof values === 'object') {
            ...
            console.log('[web] paths', paths);
            onChange(paths, parser.generateSchema(paths));
        }
    };
