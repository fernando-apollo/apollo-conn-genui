...
export const SelectionPaths = ({paths, onChange, onCollapse}: ISelectionPathsProps = {
  paths: [],
  onChange: () => {
  },
}) => {
  ...
  return <>
    <CardRoot flex='1' m={2} p={0} size='sm'>
      <CardBody display={'flex'} m={0} p={2} pt={0}>
        <EditorWrapper
          info={"The editor below shows the current OAS selection and allows you to edit the paths and try out different selections. Use the Wand icon on the right hand side to generate a schema."}
          readOnly={false}
          value={JSON.stringify(parsedPaths, null, 2)} language='json' title="Selection Paths"
          ...
          actions={[
            ...
            <IconButton
              ...
              size='xs' variant='subtle' onClick={() => {
